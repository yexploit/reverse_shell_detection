...
def parse_auth_line(line: str):
    ...
    Handles patterns like:
      ...
      Failed password for invalid user test from 10.0.0.1 port 54321 ssh2
      Failed password for ubuntu from 10.0.0.2 port 54321 ssh2
      Accepted password for ubuntu from 10.0.0.3 port 54321 ssh2
    """
    if "sshd" not in line:
        return None
    ...
    ts_str = line[:15]
    try:
        # Year is not present in log; assume current year
        now = datetime.datetime.now()
        ts = datetime.datetime.strptime(
            f"{ts_str} {now.year}", "%b %d %H:%M:%S %Y"
        )
    except ValueError:
        return None
...
def analyze_auth_log(path: str):
    # Step 1: parse lines into normalized events
    events = []
    ...
    # Step 2: sliding-window failure counts per IP
    ...
    # Detect brute-force by failure rate
    ...
    for ip, fails in fail_events_by_ip.items():
        ...
        for i, event in enumerate(fails):
            # Move window start
            while (
                event["timestamp"] - fails[start_idx]["timestamp"]
                > window
            ):
                start_idx += 1
            ...
            if count_in_window >= FAIL_THRESHOLD:
                ...
                break  # One alert per IP for this condition

    # Detect success after many failures
    for ip, successes in success_events_by_ip.items():
        fails = fail_events_by_ip.get(ip, [])
        if not fails:
            continue
        # Count how many fails before the first success
        first_success_time = successes[0]["timestamp"]
        prior_fails = [
            e for e in fails if e["timestamp"] < first_success_time
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="SSH Brute-Force Detection from auth.log"
    )
    parser.add_argument(
        "-f",
        "--file",
        required=True,
        help="Path to auth.log-style file",
    )
